				...
				<div class="instagram-grid-widget-wrapper clearfix">
					<div class="instagram-grid-widget">
						<?php foreach( $media_array as $item ) {
							echo '<div class="instagram-grid-image"><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"><img src="'. esc_url($item['thumbnail']) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"/></a></div>';
						} ?>
					</div>
				</div>
			<?php }
